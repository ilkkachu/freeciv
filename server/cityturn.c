...
void update_city_activities(struct player *pplayer)
{
  ...
  if (n > 0) {
    ...
    case GOLD_UPKEEP_MIXED:
      /* Nation pays for units, cities already paid buildings */
...
/**************************************************************************
  Check if city is in disorder, or even causes a revolution.
  Send messages about it. Return TRUE if there is disorder.
**************************************************************************/
static bool city_handle_disorder(struct city *pcity)
{
  struct player *pplayer = city_owner(pcity);
  ...
  if (! city_unhappy(pcity)) {
    /* All happy! Clean up possible old memories. */
    if (pcity->anarchy != 0) {
